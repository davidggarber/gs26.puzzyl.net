 ...
 */
const kit = {
    hasClass: (el, cls) => hasClass(el, cls),
    toggleClass: (el, cls, val) => toggleClass(el, cls, val),
    getPuzzleStatus: (puzzle, defaultStatus, puzzleList) => getPuzzleStatus(puzzle, defaultStatus, puzzleList),
    loadMetaMaterials: (puzzle, up, page) => loadMetaMaterials(puzzle, up, page),
    refreshTeamHomePage: (callback) => refreshTeamHomePage(callback),
    refillFromTemplate: (parent, tempId, args) => refillFromTemplate(parent, tempId, args),
    splitEmoji: (str) => splitEmoji(str),
    syncTeamName: () => _teamName,
    syncTeamMates: () => _teammates || [],
    syncTeamSolves: () => _teamSolves || {},
    syncRemoteUnlocked: () => _remoteUnlocked || [],
};
/**
...
function sortTable(th) {
    ...
    let tbody = document.getElementById('puzzle_list');
    ...
    for (let i = rows.length - 1; i >= 0; i--) {
        ...
        let val = cell.innerHTML + ' ' + prevOrder;
...
function loadViaIframe(urls) {
    let div = document.getElementById('iframe-loader');
    for (let url of urls) {
